...
function NormalPlayer(props) {
    ...
    const currentState = useRef("");//中间状态 切换歌词和CD
    const lyricScrollRef = useRef();//歌词滚动
    const lyricLineRefs = useRef([]);//所有歌词列表
    ...
    useEffect(() => {
        ...
        if (currentLineNum > 5) {
            // 保持当前歌词在第5条的位置
            let lineEl = lyricLineRefs.current[currentLineNum - 5].current;//滚动到前五条得位置
                                ...
                                >
                                    {
                                        currentLyric
                                            ? currentLyric.lines.map((item, index) => {
                                                lyricLineRefs.current[index] = React.createRef();//创建每条歌词得 p.txt得ref
                                                return (
                                                    <p
